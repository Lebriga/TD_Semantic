...
def p_programme(p):
    ...
    '''
    try:
        p[0] = ASTree2.AST("programme","main")
...
def p_commande(p):
    ...
    try: 
        if len(p) == 8:
            p[0] = ASTree2.AST("commande", "while")
            p[0].sons = [p[3], p[6]]
        else:
            if p[2] == "=":
                p[0] = ASTree2.AST("AFFECT", "=")
                p[0].sons = [p[1], p[3]]
            else:
                p[0] = ASTree2.AST("END", ";")
                p[0].sons =[p[1], p[3]]
...
def p_expression(p):
    ...
    try:
        if len(p) > 2:
            p[0] = ASTree2.AST("OPBIN", p[2])
            p[0].sons = [p[1], p[3]]
        else:
            if type(p[1]) == str:
                p[0] = ASTree2.AST("ID", p[1])
            else:
                p[0] = ASTree2.AST("NUMBER", p[1])
    except:
        pass
